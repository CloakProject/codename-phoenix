...
void MinerTestingSetup::TestPackageSelection(const CChainParams& chainparams, const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    std::unique_ptr<CBlockTemplate> pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, false);
    ...
    pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, false);
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    BOOST_CHECK(pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, false));
    ...
    for (unsigned int i = 0; i < sizeof(blockinfo)/sizeof(*blockinfo); ++i)
    {
        ...
        BOOST_CHECK(Assert(m_node.chainman)->ProcessNewBlock(chainparams, shared_pblock, true, shared_pblock->IsProofOfStake(), nullptr));
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, false), std::runtime_error, HasReason("bad-blk-sigops"));
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, false), std::runtime_error, HasReason("bad-txns-inputs-missingorspent"));
    m_node.mempool->clear();
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, false), std::runtime_error, HasReason("bad-cb-multiple"));
    m_node.mempool->clear();
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, false), std::runtime_error, HasReason("block-validation-failed"));
    m_node.mempool->clear();
