...
// public:
    ...
    // bool InvalidateBlock(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindex) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    else if (nActualSpacing > Params().GetConsensus().nPowTargetTimespan)
    {
        // printf(">> nActualSpacing = %"PRI64d" corrected to nTargetTimespan (900).\n", nActualSpacing);
        nActualSpacing = Params().GetConsensus().nPowTargetTimespan;
    }
...
CBlockIndex* BlockManager::AddToBlockIndex(const CBlockHeader& block)
{
    ...
    if (!ComputeNextStakeModifier(pindexNew->pprev, nStakeModifier, fGeneratedStakeModifier))
        error("AddToBlockIndex() : ComputeNextStakeModifier() failed");
 ...
 */
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime, bool fCheckPow = true)
{
    ...
    if (fCheckPow && pblock.IsProofOfWork() && !CheckProofOfWork(block.GetHash(), block.nBits, consensusParams))
...
bool BlockManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fCheckPow)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        /* If the previous block index isn't valid, determine if this block descends from any block which
         * has been foundinvalid (m_failed_blocks), then mark pindexPrev and any blocks between them as failed.
         * For example:
         *
         *                D3
         *              /
...
bool ChainstateManager::ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool fIsProofOfStake, bool* fNewBlock)
{
    AssertLockNotHeld(cs_main);
    {
        CBlockIndex *pindex = nullptr;
...
bool TestBlockValidity(BlockValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    AssertLockHeld(cs_main);
